...
const SlackChatPage: NextPage = () => {
  ...
  // NEW: Ref to hold the EventSource instance
  ...
  // NEW: Effect to manage the SSE connection
  useEffect(() => {
    ...
    if (selectedChannel) {
      // Create a new EventSource connection to our streaming API
      ...
      eventSource.onmessage = (event) => {
        ...
        setMessages(prevMessages => {
          if (prevMessages.some(msg => msg.ts === newMessage.ts)) {
            return prevMessages;
          }
  ...
  }, [selectedChannel]); // This effect re-runs whenever the selectedChannel changes
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    try {
      ...
      // No need to optimistically update here anymore because the event
      // from Slack will trigger the onmessage handler and update the state.
      setNewMessage('');
  ...
  return (
    // The JSX for the page layout remains the same as the previous step
      ...
      <main style={{ flex: 1, padding: '20px', display: 'flex', flexDirection: 'column', maxHeight: '100vh' }}>
        ...
        {selectedChannel && (
          <form onSubmit={handleSendMessage} style={{ display: 'flex', gap: '10px', flexShrink: 0 }}>
            <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder={`Message #${selectedChannel.name}`} disabled={isSending} style={{ flex: 1, padding: '10px', borderRadius: '5px', border: '1px solid #ccc' }} />
